// Frontend/src/pages/users/UserProfile.tsx
...
export default function UserProfile() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setProfile(updated);
      setForm(prev => ({ ...prev, ...updated })); // Merge
  ...
  const initials = (profile?.name || ctxUser?.name || "U")
    .split(" ")
    .map((w) => w[0])
    .join("")
    .toUpperCase()
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center gap-5 sm:gap-6 shadow-sm">
          <div className="relative group">
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-xl sm:text-2xl font-bold text-white shadow-lg overflow-hidden">
              {form.profilePicture ? (
                <img
